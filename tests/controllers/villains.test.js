...
const { villainsList, singleVillain, newVillain } = require('./mocks/villains')
const { getVillainBySlug, getAllVillains, saveNewVillain } = require('../../controllers/villains')
...
describe('Controllers - villains', () => {
  ...
  describe('getVillainBySlug', () => {
    ...
    it('retrieves the villain associated with the provided villain from the database and calls response.send with it', async () => {
      ...
      expect(stubbedSend).to.have.been.calledWith('Ursula')
    })
  })

  it('returns a 404 when no slug is found', async () => {
  ...
  describe('saveNewVillain', () => {
    it('accepts new villain details and saves them as a new villain, returning the saved villain with a 201 status', async () => {
      ...
      expect(stubbedStatusSend).to.have.been.calledWith(newVillain)
    })
  })
})
